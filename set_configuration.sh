...
docker exec -u www-data app-server php occ --no-warnings config:system:set trusted_domains 1 --value=web
...
docker exec -u www-data app-server php occ --no-warnings config:system:set onlyoffice StorageUrl --value="http://web/"
